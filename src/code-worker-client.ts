...
export function codeWorkerArguments(
  ...
  const loaderSpecifier = sourceRuntime
    ? pathToFileURL(tsxLoaderPath ?? createRequire(moduleUrl).resolve("tsx")).href
    : undefined;
  return [
...
function isCodeExecutionResult(value: unknown): value is CodeExecutionResult {
  ...
  return Number.isInteger(record.exitCode)
...
export async function runPlaywrightCodeWorker(options: CodeExecutionOptions): Promise<CodeExecutionResult> {
  ...
  return new Promise<CodeExecutionResult>((resolve, reject) => {
      ...
      void (async () => {
        try {
          const result = JSON.parse(
            await readFile(join(options.outputDir, CODE_WORKER_RESULT_FILE), "utf8"),
          ) as unknown;
          ...
          reject(new Error(
            `O worker Playwright terminou com código ${code ?? 1} sem resultado válido: ${detail || reason}`,
          ));
        }
      })();
    };
    const deadline = setTimeout(
      () => fail(
        new Error(`O worker Playwright excedeu o limite de ${options.timeoutMs + WORKER_SHUTDOWN_GRACE_MS} ms.`),
        true,
      ),
