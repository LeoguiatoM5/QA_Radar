...
if(journeyForm)journeyForm.addEventListener('submit',async event=>{
  ...
  try{
    ...
    if(!response.ok)throw new Error(data.error||'Não foi possível iniciar a jornada.');currentJourneyId=data.id;currentJourneyToken=data.accessToken;journeyCancel.hidden=false;const report=await pollJourney(data.id,data.accessToken),status=document.querySelector('#journey-status');result.hidden=false;document.querySelector('#journey-title').textContent=report.name;document.querySelector('#journey-summary').textContent=report.steps.length+' passo(s) · '+(report.durationMs/1000).toFixed(1)+'s';status.className='status '+(report.status==='passed'?'pass':'fail');status.textContent=report.status==='passed'?'APROVADA':'REPROVADA';
    document.querySelector('#journey-steps').innerHTML=report.steps.map(step=>{const base='/api/journeys/'+data.id+'/';return '<div class="issue"><span class="badge '+(step.status==='passed'?'warning':'error')+'">'+(step.status==='passed'?'OK':'Falha')+'</span><span class="category">Passo '+(step.index+1)+'</span><div class="message"><strong>'+esc(step.action)+'</strong><small> · '+step.durationMs+' ms</small>'+(step.error?'<code>'+esc(step.error)+'</code>':'')+(step.evidence?'<div class="journey-step-links"><a href="'+base+encodeURIComponent(step.evidence.before)+'" target="_blank">Antes</a><a href="'+base+encodeURIComponent(step.evidence.after)+'" target="_blank">Depois</a></div>':'')+'</div></div>'}).join('');result.scrollIntoView({behavior:'smooth',block:'start'});
  }catch(reason){error.textContent=reason.message;error.style.display='block'}finally{currentJourneyId=undefined;currentJourneyToken=undefined;journeyCancel.hidden=true;journeyCancel.disabled=false;journeyCancel.textContent='Cancelar jornada';submit.disabled=false;submit.textContent='Executar jornada'}
});`;
