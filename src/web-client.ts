...
if(journeyForm)journeyForm.addEventListener('submit',async event=>{
  ...
  }catch(reason){error.textContent=reason.message;error.style.display='block'}finally{submit.disabled=false;submit.textContent='Executar jornada'}
});`;
