...
journeyCredentials.innerHTML='<strong style="display:block;font-size:.82rem">Credenciais desta execução</strong><span style="display:block;color:var(--muted);font-size:.75rem;margin-top:4px">Os valores serão usados somente neste job e não serão salvos.</span><div class="journey-credential-fields"></div>';
journeyJson?.parentElement?.insertBefore(journeyCredentials, journeyJson.nextElementSibling);
function refreshJourneyCredentials(){
  const fields=journeyCredentials.querySelector('.journey-credential-fields');
  if(!fields||!journeyJson)return;
  let names=[];try{const definition=JSON.parse(journeyJson.value);names=[...new Set((definition.steps||[]).map(step=>step.valueFromInput).filter(name=>typeof name==='string'&&/^QA_RADAR_INPUT_[A-Z0-9_]+$/.test(name)))]}catch{names=[]}
  fields.innerHTML=names.map(name=>'<label style="margin:12px 0 5px" for="journey-input-'+name+'">'+esc(name.replace('QA_RADAR_INPUT_',''))+'</label><input id="journey-input-'+name+'" data-journey-input="'+name+'" type="password" autocomplete="new-password" required placeholder="Senha ou credencial">').join('');
  journeyCredentials.style.display=names.length?'block':'none';
}
...
journeyProgress.setAttribute('aria-live','polite');
journeyProgress.style.cssText='display:none;align-items:center;gap:8px;color:var(--muted);font-size:.82rem;margin-top:12px';
journeyProgress.innerHTML='<i class="loader"></i><span></span>';
journeyCancel?.parentElement?.append(journeyProgress);
const journeyEvidenceButton=document.querySelector('#journey-evidence-button');
...
if(journeyEvidenceButton)journeyEvidenceButton.addEventListener('click',()=>journeyEvidenceModal.showModal());
...
if(journeyEvidenceForm)journeyEvidenceForm.addEventListener('submit',async event=>{event.preventDefault();if(!completedJourneyId||!completedJourneyToken)return;const submit=journeyEvidenceForm.querySelector('button[type=submit]'),error=document.querySelector('#journey-evidence-error'),reportWindow=window.open('/','_blank');error.style.display='none';submit.disabled=true;submit.innerHTML='<i class="loader"></i>Gerando HTML';try{const response=await fetch('/api/journeys/'+completedJourneyId+'/evidence-report',{method:'POST',headers:{...journeyHeaders(completedJourneyToken),'content-type':'application/json'},body:JSON.stringify({testerName:document.querySelector('#journey-tester-name').value,testType:document.querySelector('#journey-test-type').value})}),data=await response.json();if(!response.ok)throw new Error(data.error||'Não foi possível gerar o relatório.');journeyEvidenceModal.close();if(reportWindow)reportWindow.location.href=data.url;else window.open(data.url,'_blank')}catch(reason){reportWindow?.close();error.textContent=reason.message;error.style.display='block'}finally{submit.disabled=false;submit.textContent='Gerar HTML'}});
if(journeyForm)journeyForm.addEventListener('submit',async event=>{
  event.preventDefault();const submit=document.querySelector('#journey-submit'),error=document.querySelector('#journey-error'),result=document.querySelector('#journey-results');error.style.display='none';completedJourneyId=undefined;completedJourneyToken=undefined;journeyEvidenceButton.hidden=true;submit.disabled=true;submit.innerHTML='<i class="loader"></i>Executando jornada';journeyProgress.style.display='flex';journeyProgress.querySelector('span').textContent='Preparando jornada…';
  try{
    ...
    const inputSecrets=Object.fromEntries([...journeyCredentials.querySelectorAll('[data-journey-input]')].map(input=>[input.dataset.journeyInput,input.value]));
    const response=await fetch('/api/journeys',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({url:document.querySelector('#journey-url').value,browser:document.querySelector('#journey-browser').value,timeoutMs:Number(document.querySelector('#journey-timeout').value),journey,inputSecrets})}),data=await response.json();
    if(!response.ok)throw new Error(data.error||'Não foi possível iniciar a jornada.');currentJourneyId=data.id;currentJourneyToken=data.accessToken;submit.hidden=true;journeyCancel.hidden=false;journeyProgress.querySelector('span').textContent='Executando jornada…';const report=await pollJourney(data.id,data.accessToken),status=document.querySelector('#journey-status');completedJourneyId=data.id;completedJourneyToken=data.accessToken;journeyCancel.hidden=true;submit.hidden=false;submit.disabled=false;submit.textContent='Executar nova jornada';journeyEvidenceButton.hidden=false;result.hidden=false;document.querySelector('#journey-title').textContent=report.name;document.querySelector('#journey-summary').textContent=report.steps.length+' passo(s) · '+(report.durationMs/1000).toFixed(1)+'s';status.className='status '+(report.status==='passed'?'pass':'fail');status.textContent=report.status==='passed'?'APROVADA':'REPROVADA';
  ...
  }catch(reason){error.textContent=reason.message;error.style.display='block'}finally{currentJourneyId=undefined;currentJourneyToken=undefined;journeyCancel.hidden=true;journeyCancel.disabled=false;journeyCancel.textContent='Cancelar jornada';journeyProgress.style.display='none';journeyCredentials.querySelectorAll('input').forEach(input=>{input.value=''});submit.hidden=false;submit.disabled=false;if(submit.textContent.includes('Executando'))submit.textContent='Executar jornada'}
});`;
