...
function evidence(step: JourneyRunResult["steps"][number]): string {
  ...
  const before = encodeURIComponent(step.evidence.before), after = encodeURIComponent(step.evidence.after);
...
export function createJourneyEvidenceHtml(report: JourneyRunResult, metadata: JourneyEvidenceMetadata, generatedAt = new Date()): string {
