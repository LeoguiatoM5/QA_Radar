...
export async function createJourneyEvidenceHtml(report: JourneyRunResult, metadata: JourneyEvidenceMetadata, readAsset: AssetReader, generatedAt = new Date()): Promise<string> {
  const stepArticles = await Promise.all(
    ...
    <p class="technical">Ação: <code>${escapeHtml(step.action)}</code>${step.durationMs === undefined ? "" : ` · ${Math.round(step.durationMs)} ms`}</p>
    ${step.error ? `<p class="error">${escapeHtml(friendlyStepFailureMessage(step.action))}</p><pre class="error-detail">${escapeHtml(step.error)}</pre>` : ""}
