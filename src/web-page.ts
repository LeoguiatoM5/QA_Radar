...
export function createApiTestsPage(allowCodeMode = false): string {
  return `<!doctype html>
<html lang="pt-BR">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="QA Radar - Testes de API"><title>QA Radar · Testes de API</title><style>${WEB_STYLES}${NAV_RESPONSIVE_STYLES}${CODE_MODE_STYLES}</style></head>
<body>${renderApiPage(allowCodeMode)}</body>
<script>${WEB_CLIENT_SCRIPT}</script></html>`;
}
