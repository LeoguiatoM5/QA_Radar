...
const FORBIDDEN_IDENTIFIERS = new Set(["process", "require", "eval", "global", "globalThis", "Function"]);
...
function scan(source: string): Token[] {
  const scanner = createScanner(ScriptTarget.Latest, true, LanguageVariant.Standard, source);
  const tokens: Token[] = [];
  for (;;) {
    const kind = scanner.scan();
    if (kind === SyntaxKind.EndOfFileToken) return tokens;
    tokens.push({
      kind,
...
function assertAllowedModule(name: string): void {
  if (!ALLOWED_HOSTED_IMPORTS.has(name)) {
    throw new Error(`Importação não permitida na execução hospedada: ${name}. Use somente a API oficial do Playwright.`);
  }
}
...
export function assertHostedPlaywrightCode(source: string): void {
  ...
  for (const [index, token] of tokens.entries()) {
    ...
    if (token.text === "import" && next?.kind === SyntaxKind.OpenParenToken) {
      throw new Error("Importações dinâmicas não são permitidas na execução hospedada.");
    }
    if ((token.kind === SyntaxKind.StringLiteral || token.kind === SyntaxKind.NoSubstitutionTemplateLiteral) && token.value === "constructor") {
      throw new Error("Acesso dinâmico a constructor não é permitido na execução hospedada.");
    }
    if (token.kind === SyntaxKind.StringLiteral && (previous?.text === "from" || previous?.text === "import")) {
