...
export function renderApiPage(allowCodeMode: boolean): string {
  const enabled = allowCodeMode;
  return `<main class="shell">
  ${renderAppNav("api")}
  ${enabled ? `${renderToolHeader("Automação Playwright", "Testes de API", "Escreva chamadas HTTP com o fixture request do Playwright Test e execute no ambiente configurado.")}<section class="journey-workspace">${renderApiPanel()}</section>${renderEvidenceModal()}` : '<section class="panel"><div class="eyebrow">Testes de API</div><h1>Recurso indisponível neste ambiente</h1><p class="lead">A execução de testes de API ainda não está habilitada nesta implantação.</p><p><a class="home-action" href="/docs"><strong>Consultar configuração</strong><span>Veja os requisitos de infraestrutura para habilitar o recurso.</span></a></p></section>'}
  <footer>&copy; 2026 QA Radar · Todos os direitos reservados.</footer>
</main>`;
}
...
function renderApiPanel(): string {
  const placeholder =
    "import { test, expect } from '@playwright/test';\n\ntest('exemplo de API', async ({ request }) => {\n  const apiResponse = await request.get('https://sua-api.exemplo.com/endpoint');\n  expect(apiResponse.status()).toBe(200);\n});";
  return `<section class="panel" id="code-mode-panel"><div class="eyebrow">Testes de API</div><h2>Teste de API em TypeScript</h2><p class="sub">Escreva ou cole um arquivo Playwright Test usando o fixture <code>request</code> para chamadas HTTP diretas, sem depender do navegador.</p><div class="code-editor-head"><div><label for="playwright-code">Arquivo Playwright</label><small>qa-radar.spec.ts</small></div><label class="code-import" for="code-import">Importar arquivo</label><input id="code-import" type="file" accept=".ts,.spec.ts" hidden></div><textarea id="playwright-code" rows="18" spellcheck="false" aria-label="Código do teste de API" placeholder="${placeholder.replaceAll('"', "&quot;")}"></textarea><div class="journey-controls code-actions"><button id="code-api-step" class="secondary" type="button">Adicionar passo de API</button><button id="code-save" class="secondary" type="button">Exportar .spec.ts</button><button id="code-execute" type="button">Executar</button></div><div id="code-result" hidden aria-live="polite"></div><p class="hint">Este recurso executa código informado pelo usuário. Revise o arquivo antes de executar.</p></section>`;
}
